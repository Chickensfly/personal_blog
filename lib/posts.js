...
/**
 * Reads content/about.md and splits it into individual lines —
 * used by the landing page's line-by-line cascade reveal.
 *
 * Every non-empty line in the source file becomes its own "flap".
 * Inline markdown (links, bold, italics) still renders; lines just
 * aren't wrapped in <p> tags so each can be its own small element.
 */
export function getAboutLines() {
  const filepath = path.join(CONTENT_DIR, 'about.md');
  if (!fs.existsSync(filepath)) {
    return ['Add your text to <code>content/about.md</code>.'];
  }
  const { content } = matter(fs.readFileSync(filepath, 'utf-8'));
